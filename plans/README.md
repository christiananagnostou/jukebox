...
Reconciled by the `improve` audit on 2026-08-26. Execute focused plans 007-040 in the order below unless dependencies say otherwise. Each executor must read its plan fully, honor its STOP conditions, run every verification gate, and update the status row when done.
...
| [004](004-playlists-queue-and-history.md)         | Durable collections, queue, and history                         | IN PROGRESS — stable native queue entries, desktop parity, committed session/queue persistence, and the named manual-playlist repository are complete; the bounded playlist workspace, editable queue, and listening-history foundation are active in plans 038-040, while smart rules and broader collection workflows remain |
