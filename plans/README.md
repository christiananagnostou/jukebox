...
Reconciled by the `improve` audit on 2026-08-26. Execute focused plans 007-038 in the order below unless dependencies say otherwise. Each executor must read its plan fully, honor its STOP conditions, run every verification gate, and update the status row when done.
...
| [004](004-playlists-queue-and-history.md)         | Durable collections, queue, and history                         | IN PROGRESS — stable native queue entries, desktop parity, committed session/queue persistence, and the named manual-playlist repository are complete; the bounded playlist workspace is active in plan 038, while reordering, smart rules, history, and broader collection workflows remain |
