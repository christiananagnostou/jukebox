...
| [004](004-playlists-queue-and-history.md)         | Durable collections, queue, and history                         | IN PROGRESS — stable native queue entries, desktop parity, committed session/queue persistence, the named manual-playlist repository, and bounded history-powered built-ins are complete; the playlist workspace, editable queue, listening-history foundation, and built-in workspace await native QA in plans 038-040 and 042, while editable smart rules and broader collection workflows remain |
